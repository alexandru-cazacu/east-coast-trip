<?php
        ...
        while (!empty($LEFT_NODES)) {
            ...
            foreach ($routes[$cityNames[$lowestWeightIndex]] as $key => $val) {
                ...
                if ($isPlane && isset($val["airplane"]))
                    $vehicle = "airplane";
                if ($isTrain && isset($val["train"]))
                    $vehicle = "train";
                if ($isBus && isset($val["bus"]))
                    $vehicle = "bus";
    ...
    // $result = getShortestPath(1, 5, $routes, $cityNames, $isBus, $isPlane, $isTrain, true, $isFast);
    $result = getShortestPath(1, 3, $routes, $cityNames, false, true, false, true, false);
